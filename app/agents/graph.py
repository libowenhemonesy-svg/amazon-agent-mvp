...
ADS_ALERTS = {"acos_high", "clicks_without_orders"}
...
def _default_causes(agent_name: str) -> list[str]:
    ...
    if agent_name == "ads_agent":
        return ["关键词匹配过宽", "无效点击增加", "Listing 转化下降", "广告出价结构异常"]
...
def _diagnostic_checks(agent_name: str) -> list[str]:
    ...
    if agent_name == "ads_agent":
        return ["高花费搜索词", "点击无订单关键词", "CTR/CVR 是否低于历史", "广告预算和出价变化"]
