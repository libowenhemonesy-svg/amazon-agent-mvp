...
from app.db.models import AlertTask, ProfitCalculation, SalesDaily
...
# 由 main.py 初始化
_listing_llm_client = None


def init_listing_llm_client(client) -> None:
    """初始化 Listing 生成所用的大模型客户端。"""
    global _listing_llm_client
    _listing_llm_client = client
...
def optimize_listing(payload: ListingOptimizeRequest) -> dict:
    ...
    if _listing_llm_client is None:
        raise HTTPException(status_code=503, detail="未配置真实 LLM，无法生成 Listing 文案")

    try:
        result = ProductListingAgent(llm_client=_listing_llm_client).run(
            {"entities": {"keyword": _keyword_text(payload.keywords)}},
            research_context={"product_context": product_description},
        )
        generated = result.get("listing") or {}
        bullets = generated.get("bullet_points") or []
        if not generated.get("title") or len(bullets) != 5:
            raise ValueError("真实 LLM 没有返回完整的 Listing 文案")
        listing = {
            "title": generated["title"],
            "bullets": bullets,
            "description": generated.get("description", ""),
            "search_terms": generated.get("search_terms", ""),
            "backend_terms": generated.get("search_terms", "").split(),
        }
        evaluation = listing_optimizer.evaluate_listing(
            listing=listing,
        ...
        return {
            ...
            "competitor_asin": payload.competitor_asin.strip(),
            **evaluation,
        ...
        status = 503 if "未配置真实 LLM" in str(exc) else 502
        raise HTTPException(status_code=status, detail=f"Listing 大模型调用失败: {exc}") from exc


def _keyword_text(keywords: str | list[str]) -> str:
